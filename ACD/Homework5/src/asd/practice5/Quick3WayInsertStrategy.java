...
public class Quick3WayInsertStrategy<T> implements SortStrategy<T> {
    ...
    private void sortBoth(Comparable<T>[] array,Comparator<T> comparator,  int l, int r) {
        if (l >= r)
            return;
        ...
        while (i <= rEdge) {
            if (comparator.compare((T)middle,(T)array[i])>0) {
                exch(array, lEdge, i);
                i++;
                lEdge++;
            } else if (middle ==  array[i]) {
                i++;
            } else if (comparator.compare((T)middle,(T)array[i])<0) {
                exch(array, rEdge, i);
                rEdge--;
            }
        }

        sortBoth(array,comparator,  l, lEdge - 1);
        sortBoth(array, comparator, rEdge + 1, r);

    }

    private void sortBothDesc(Comparable<T>[] array,Comparator<T> comparator,  int l, int r) {
        if (l >= r)
            return;
        ...
        sortBothDesc(array,comparator,  l, lEdge - 1);
        sortBothDesc(array,comparator,  rEdge + 1, r);

    }

    private void sortBothDesc(Comparable<T>[] array,  int l, int r) {
        if (l >= r)
            return;
        ...
        sortBothDesc(array,  l, lEdge - 1);
        sortBothDesc(array,  rEdge + 1, r);
